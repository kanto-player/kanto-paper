...
\begin{itemize}
	...
	\item Once the processor gets control, it reads in the first block, which
		...
		each song on the \texttt{SD card}. The processor then seeks to the 
...
The next 60 bytes is a null-terminated string with the track title.
...
controls a framebuffer that is used to fill in the top part of the VGA display.
It uses this framebuffer, along with the PS/2 keyboard, to provide a user interface.
This interface shows the different track titles and allows you to select from 
them using the keyboard. If a new track is selected using the keyboard controls, 
the program takes control away from the conductor, changes the block address 
to the start of the selected track, reads in that block, and then returns 
